...
void with_mpi() {
    ...
    for (int part = bits_length - 1; part >= 0; part--) { // 每个数据计数
        // 初始化计数数组
        if (my_rank == 0) {
            memset(cnt[0], 0, (UINT16_MAX + 1) * sizeof(int));
            memset(cnt[comm_sz], 0, (UINT16_MAX + 1) * sizeof(int));
        } else {
            memset(cnt[my_rank], 0, (UINT16_MAX + 1) * sizeof(int));
        }
        // 同步 indies 辅助数组
        ...
        // 合并 cnt 处理前缀和
        if (my_rank != 0) {
            MPI_Send(cnt[my_rank], UINT16_MAX + 1, MPI_INT, 0, 1, MPI_COMM_WORLD);
            MPI_Recv(cnt[my_rank], UINT16_MAX + 1, MPI_INT, 0, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        } else {
            // 收集 cnt 数组
            for (int i = 1; i < comm_sz; i++) {
                MPI_Recv(cnt[i], UINT16_MAX + 1, MPI_INT, i, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
            ...
            // 分发 cnt 数组
            for (int i = 1; i < comm_sz; i++) {
                MPI_Send(cnt[i], UINT16_MAX + 1, MPI_INT, i, 2, MPI_COMM_WORLD);
            }
        }
